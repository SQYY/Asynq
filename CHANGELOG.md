...
- Fixed overflow issue with 32-bit systems (For details, see https://github.com/SQYY/Asynq/pull/426)
...
- Introduced new configuration for `DelayedTaskCheckInterval`. See [godoc](https://godoc.org/github.com/SQYY/Asynq) for more details.
...
- Fixed latency issue around memory usage (see https://github.com/SQYY/Asynq/issues/309).
...
- Free unique lock when task is deleted (https://github.com/SQYY/Asynq/issues/275).
...
**IMPORTATNT**: All `Inspector` related code are moved to subpackage "github.com/SQYY/Asynq/inspeq"
...
- `Scheduler` type is added to enable periodic tasks. See the godoc for its APIs and [wiki](https://github.com/SQYY/Asynq/wiki/Periodic-Tasks) for the getting-started guide.
...
**IMPORTANT**: If you are upgrading from a previous version, please install the latest version of the CLI `go get -u github.com/SQYY/Asynq/tools/asynq` and run `asynq migrate` command. No process should be writing to Redis while you run the migration command.
...
- Fixes issue of same tasks processed by more than one worker (https://github.com/SQYY/Asynq/issues/90).

## [0.9.3] - 2020-06-12

### Fixed

- Fixes the JSON number overflow issue (https://github.com/SQYY/Asynq/issues/166).
...
- [Fixed cancelfunc leak](https://github.com/SQYY/Asynq/pull/145)
